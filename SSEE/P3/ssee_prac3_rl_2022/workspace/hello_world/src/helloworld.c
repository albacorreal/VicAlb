...
int main()
{
		...
		while(1)
		{
			...
			if (data != 0)
			{
				...
				// Realizamos una máscara ya que los cuatro bits menos significativos del código ASCII
				// se corresponden con el número binario al que representan
				tecla &= 0x0F;
				...
				if (((tecla >= 0) && (tecla < 10)) || (tecla == 65) || (tecla == 97))
				{
					...
					// Ponemos en valor HIGH la variable tecla_pulsada
					tecla_pulsada = 1;

					// Enviamos por el GPO2 el valor de tecla_pulsada
					XIOModule_DiscreteWrite(&iomodule, 2, tecla_pulsada);

					// Ponemos en valor LOW la variable tecla_pulsada
					tecla_pulsada = 0;
				...
				{
					// La tecla pulsada no es ninguna de las reconocidas como válida
			...
			if (intc_ext == 1)
			{
				...
				// Usamos una máscara para obtener del GPI1 la parte real de la multiplicación
				result_BCD_real = GPI1 & 0xFFFF0000;

				// Usamos máscaras para dividir la parte real en decenas, unidades, décimas y centésimas
				buf_real[3] = result_BCD_real & 0xF000;
				buf_real[2] = result_BCD_real & 0x0F00;
				buf_real[1] = result_BCD_real & 0x00F0;
				...
				// Usamos máscaras para dividir el resultado real en decenas, unidades, décimas y centésimas
				buf_imag[3] = result_BCD_imag & 0xF000;
				buf_imag[2] = result_BCD_imag & 0x0F00;
				buf_imag[1] = result_BCD_imag & 0x00F0;
				...
				if (data != 0)
				{
					...
					if ((tecla == 67) || (tecla == 99))
					{
						...
						// Enviamos por el GPO1 el código de tecla
						XIOModule_DiscreteWrite(&iomodule, 1, cod_tecla);
			...
			// Definimos el que será el contador del número de interrupciones y el valor del led
			u8 contador = 0, led;
