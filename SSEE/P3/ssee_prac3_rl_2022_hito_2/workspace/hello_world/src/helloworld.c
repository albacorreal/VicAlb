...
int main()
{
		...
		// Definición de variables para mostrar el número tecleado
		u8 unid = 0, decim = 0, count = 0;
		// Definición del semáforo para la espera de la letra 'C'
		u8 esperaC = 0;
		...
		while(1)
		{
			...
			if (data != 0)
			{
				...
				// Comprobación de que la tecla pulsada es un número
				if ((rx_buf >= 48) && (rx_buf <= 57))
				{
					// La tecla pulsada es un número entre el '0' y el '9'
					// Se comprueba si se espera o no la letra 'C', que indica el valor de entZ_sal
					// Si no se espera, se realiza este if cuya finalidad es para mostrar los datos de entrada
					if (esperaC == 0)
					{
						if (count != 0)
						{
							// Se ha introducido un número antes
					...
					// Asignamos a "cod_tecla" el valor de la tecla pulsada
					cod_tecla = decim;
				...
				// Comprobación de que la tecla pulsada es la letra 'A'
				else if ((rx_buf == 65) || (rx_buf == 97))
				{
					// La tecla pulsada es la 'A'
					// Asignamos a "cod_tecla" el valor de la tecla pulsada
					cod_tecla = 0xA;
					...
					// Al igual que antes, se comprueba el estado de entZ_sal
					if (esperaC == 0)
					{
				...
				// Comprobación de que la tecla pulsada es la letra 'C'
			...
			if (intc_ext == 1)
			{
				...
				result_BCD_real[3] = (GPI1 >> 28);			// Decenas de la parte real
				result_BCD_real[2] = (GPI1 >> 24) & 0x0F;	// Unidades de la parte real
				result_BCD_real[1] = (GPI1 >> 20) & 0x0F;	// Décimas de la parte real
				result_BCD_real[0] = (GPI1 >> 16) & 0x0F;	// Centésimas de la parte real

				// Haciendo uso de máscaras y desplazamientos obtenemos las cuatro cifras BCD de la parte imaginaria
				result_BCD_imag[3] = (GPI1 >> 12) & 0x0F;	// Decenas de la parte imaginaria
				result_BCD_imag[2] = (GPI1 >>  8) & 0x0F;	// Unidades de la parte imaginaria
				result_BCD_imag[1] = (GPI1 >>  4) & 0x0F;	// Décimas de la parte imaginaria
				result_BCD_imag[0] = GPI1 & 0x0F;			// Centésimas de la parte imaginaria
